...
function copyRows(rowStart, rowEnd) {
	excel.workBooks(mainName).activate();
	excel.activeWorkbook.activeSheet.Rows(rowStart + ":" + rowEnd).select();
	excel.selection.copy();
}
function pasteRow(name, rowForInsert){
	excel.workbooks(name).activate();
	excel.activeWorkbook.activeSheet.rows(rowForInsert).select();//here
...
function injectSelect (sel, rowsObject) {
	...
	for (x in rowsObject) {
		...
		if(rowsObject[x] == "Представительство") {
			opt.selected = true;
		}
		sel.appendChild(opt);
	}
